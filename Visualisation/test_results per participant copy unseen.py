...
for test in tests:
    ...
    for subj_id in unique_subj_ids:
        ...
        #subject_accuracies[subj_id] = accuracy_score(true_subj, pred_subj)
        #subject_errs[subj_id] = 0.0
        ...
        #subject_errs[subj_id] = np.mean(np.percentile(bootstrap_accs, 97.5) - np.percentile(bootstrap_accs, 2.5))/2
        ...
        # Divide error by two as providing the full range of the confidence interval in the bar chart incorrectly sh
...
for i, (model_acc_dict, model_lower_dict, model_upper_dict) in enumerate(zip(per_subj_accs, per_subj_lower, per_subj_upper)):
    ...
    #ax.bar(x + offset, accuracies, width=width, label=labels[i], color=colors[i % len(colors)], zorder=2, alpha=1)
